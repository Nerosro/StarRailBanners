...
function createVersionDiv(jsonData, rootDiv) {
    for (const entry of jsonData) {
        ...
        const lastMinorVersionNumber = banners[banners.length - 1]["minor_version"]

        //console.log("major version: " + version)
        console.log(banners)

        const {collapseButton, versionDiv} = createHtmlElements(majorVersionNumber, lastMinorVersionNumber);
...
function createHtmlElements(majorVersionNumber, minorVersionNumber) {
    ...
    collapseButton.innerHTML = "Banners during version " + majorVersionNumber + ".0 -> version " + majorVersionNumber + "." + minorVersionNumber;
